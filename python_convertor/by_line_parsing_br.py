...
res = open('result_reg.mediawiki', 'w+')
...
while True:
    ...
    if re.match(r'{\|\s+class="wikitable"', line):
        page = etree.Element('table')
        doc = etree.ElementTree(page)
        headElt = etree.SubElement(page, 'tbody')
        ...
        my_str = my_str = etree.tostring(doc, pretty_print=True, encoding='utf-8').decode('utf-8')
        print (type(my_str))
        res.write("%s\n" % my_str)
    else:
    #    res.write(line)
        #if line.startswith('<syntaxhighlight'):
        if re.match(r'\s*<syntaxhighlight', line):
            print("Inside:",line)
            res.write(line)
